...
public class GuiceFactory implements FactoryBean<Injector> {
	...
	public Injector getObject() throws Exception {
		...
		try {
			...
			if (hosts.length == 0) {
				throw new RuntimeException(
						"Fatal error: no Cassandra hosts configured");
			}

			for (CassandraHost host : hosts) {
				if (StringUtils.isEmpty(host.getHost())) {
					throw new RuntimeException(
							"Fatal error: Cassandra hostname cannot be empty");
				}
			...
			cpProps.put("cassandra.cluster_name",
					getAndValidateProperty("cassandra.cluster"));
		...
		if (applicationContext.containsBean("serviceModule")) {
			Module serviceModule = (Module) applicationContext
					.getBean("serviceModule");
			moduleList.add(serviceModule);
		}
		// Bentobox - Added guice aop logging intercepter. - START
		// Module guiceAopModule = (Module) applicationContext
		// .getBean("guiceAopModule");
		// moduleList.add(guiceAopModule);
		// Bentobox - Added guice aop logging intercepter. - END
	...
	private String getAndValidateProperty(final String propName) {
		...
		Preconditions.checkNotNull(propValue, propName
				+ " cannot be unset. Set this in your properties");
