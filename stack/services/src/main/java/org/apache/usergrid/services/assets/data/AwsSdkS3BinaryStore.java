...
public class AwsSdkS3BinaryStore implements BinaryStore {
	...
	private static final Logger		logger	= LoggerFactory
													.getLogger(AwsSdkS3BinaryStore.class);
	...
	// TODO: GREY rework how the s3 client works because currently it handles
	// initlization and returning of the client
	// ideally it should only do one. and the client should be initlized at the
	 ...
	 * if (bucketName == null) { logger.error(
	 * "usergrid.binary.bucketname  not properly set so amazon bucket is null");
	 * throw new AwsPropertiesNotFoundException( "usergrid.binary.bucketname");
	 * 
	 * }
	 * 
	 * final UsergridAwsCredentialsProvider ugProvider = new
	 * UsergridAwsCredentialsProvider(); AWSCredentials credentials =
	 * ugProvider.getCredentials(); ClientConfiguration clientConfig = new
	 * ClientConfiguration(); clientConfig.setProtocol(Protocol.HTTP);
	 * 
	 * s3Client = new AmazonS3Client(credentials, clientConfig); if (regionName
	 * != null)
	...
	private AmazonS3 getS3Client() throws Exception {
		this.accessId = this.properties.getProperty("AWS_ACCESS_KEY_ID");
		if (this.accessId == null) {
			logger.error("AWS_ACCESS_KEY_ID not properly set so amazon access key is null");
			throw new AwsPropertiesNotFoundException("AWS_ACCESS_KEY_ID");
		}
		this.secretKey = this.properties.getProperty("AWS_SECRET_KEY");
		if (this.secretKey == null) {
			logger.error("AWS_SECRET_KEY not properly set so amazon secret key is null");
			throw new AwsPropertiesNotFoundException("AWS_SECRET_KEY");
		}
		this.bucketName = this.properties
				.getProperty("usergrid.binary.bucketname");
		if (this.bucketName == null) {
			logger.error("usergrid.binary.bucketname  not properly set so amazon bucket is null");
			throw new AwsPropertiesNotFoundException(
					"usergrid.binary.bucketname");
		}
		AWSCredentials credentials = new BasicAWSCredentials(this.accessId,
				this.secretKey);
		...
		if (this.regionName != null) {
			this.s3Client.setRegion(Region.getRegion(Regions
					.fromName(this.regionName)));
		}
		return this.s3Client;
	}

	@Override
	public void write(final UUID appId, final Entity entity,
		...
		final Map<String, Object> fileMetadata = AssetUtils
				.getFileMetadata(entity);
		...
		if (written < FIVE_MB) { // total smaller than 5mb
			...
			result = getS3Client().putObject(bucketName, uploadFileName,
					awsInputStream, om);

			String md5sum = Hex.encodeHexString(Base64.decodeBase64(result
					.getContentMd5()));
			...
			written = 0; // reset written to 0, we still haven't wrote anything
							// in fact
			int partNumber = 1;
			int firstByte = 0;
			...
			InitiateMultipartUploadResult initResponse = getS3Client()
					.initiateMultipartUpload(initRequest);

			InputStream firstChunck = new ByteArrayInputStream(data);
			PushbackInputStream chunckableInputStream = new PushbackInputStream(
					inputStream, 1);
			...
			while (-1 != (firstByte = chunckableInputStream.read())) {
				...
				File tempFile = File.createTempFile(entity.getUuid().toString()
						.concat("-part").concat(String.valueOf(partNumber)),
						"tmp");
				...
				try {
					...
					if (isFirstChunck == true) {
						partSize = IOUtils.copyLarge(firstChunck, os, 0,
								(FIVE_MB));
						isFirstChunck = false;
					} else {
						partSize = IOUtils.copyLarge(chunckableInputStream, os,
								0, (FIVE_MB));
					}
					written += partSize;
				...
				Boolean isLastPart = -1 == (firstByte = chunckableInputStream
						.read());
				if (!isLastPart)
					chunckableInputStream.unread(firstByte);
				...
				UploadPartRequest uploadRequest = new UploadPartRequest()
						...
						.withBucketName(bucketName).withKey(uploadFileName)
						.withInputStream(chunk).withPartNumber(partNumber)
						.withPartSize(partSize).withLastPart(isLastPart);
				partETags.add(getS3Client().uploadPart(uploadRequest)
						.getPartETag());
				partNumber++;
			}
			...
			if (overSizeLimit) {
				...
				MultipartUploadListing listResult = getS3Client()
						.listMultipartUploads(listRequest);

				// upadte the entity with the error.
				try {
					logger.error("starting update of entity due to oversized asset");
					fileMetadata.put("error",
							"Asset size is larger than max size of "
									+ maxSizeBytes);
					em.update(entity);
				} catch (Exception e) {
				...
				while (listResult.getMultipartUploads().size() != 0
					...
					logger.debug("Files that haven't been aborted are: ",
							listResult.getMultipartUploads().listIterator()
									.toString());

				}
				if (timesIterated == 0) {
					logger.error("Files parts that couldn't be aborted in 20 seconds are:");
					Iterator<MultipartUpload> multipartUploadIterator = listResult
							.getMultipartUploads().iterator();
				...
				CompleteMultipartUploadRequest request = new CompleteMultipartUploadRequest(
						bucketName, uploadFileName, initResponse.getUploadId(),
						partETags);
				CompleteMultipartUploadResult amazonResult = getS3Client()
	...
	public InputStream read(UUID appId, Entity entity, long offset, long length)
		...
		S3Object object = getS3Client().getObject(bucketName,
				AssetUtils.buildAssetKey(appId, entity));
		...
		if (offset == 0 && length == FIVE_MB) {
			...
			object.getObjectContent().read(data, Ints.checkedCast(offset),
	...
	public void delete(UUID appId, Entity entity) throws Exception {
		getS3Client().deleteObject(
				new DeleteObjectRequest(bucketName, AssetUtils.buildAssetKey(
						appId, entity)));
	}
}
