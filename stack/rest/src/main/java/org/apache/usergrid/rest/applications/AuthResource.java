...
public class AuthResource extends AbstractContextResource {
    ...
    // TODO --Test Google Auth Provider--
 //   @CheckPermissionsForPath
    @GET
    @Path("google")
    public Response authGOOGLE( @Context UriInfo ui, @QueryParam("google_access_token") String google_access_token,
                            @QueryParam("ttl") long ttl, @QueryParam("callback") @DefaultValue("") String callback )
            throws Exception {

        logger.info( "AuthResource.authGOOGLE" );

        try {
            if ( StringUtils.isEmpty( google_access_token ) ) {
                return missingTokenFail( callback );
            }
            SignInAsProvider googleProvider = signInProviderFactory.google( services.getApplication() );
            User user = googleProvider.createOrAuthenticate( google_access_token );
